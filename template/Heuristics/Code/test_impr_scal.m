function test_impr_scal(x,y,NVAR,parameters,dataset_file)
%myFun - Description
%
...
% MAXGEN=30;		% Maximum no. of generations
...
table_path=sprintf("%s/Results_scal_%s.csv",base_dir,filename);
...
for j=1:length(NIND)
    ...
    running_results_path=sprintf("%s/Running_Res_%s_%d.mat", base_dir,filename,j);
    for i=1:length(IMPR)
        nimprovements = 0;
        for n=1:N_EXPERIMENTS
            [Best_vector(n), best(n,:), improvements] = run_ga_return_path(x, y, NIND(j), MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, CROSS_OP, MUT_OP, IMPR(i), STOP_EPOCHS);
